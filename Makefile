...
KBUILD_CFLAGS	:= -fno-common -fmessage-length=0 -Wall \
		   -fno-exceptions -ffunction-sections -fsigned-char \
...
LIB_LDFLAGS	:=
CFLAGS_IMAGE	:= -static
#LDFLAGS_IMAGE	:= --cref --build-id -X
LDFLAGS_IMAGE	:= --build-id -X
...
targets := $(LDS_FILE) $(IMAGE_FILE) $(IMAGE_BIN) $(IMAGE_HEX) $(IMAGE_LST)
...
# Defaults to $(IMAGE_BIN) and $(IMAGE_HEX)
all: $(IMAGE_BIN) $(IMAGE_HEX) ;
...
IMAGE-deps := $(LDS_FILE) $(IMAGE_INIT) $(IMAGE_MAIN) 

IMAGE_VER  := build_time.o
...
quiet_cmd_image_ver = CC      $(IMAGE_VER)
      cmd_image_ver = $(CC) $(filter-out -Werror=date-time, \
                             ...
                             $(NOSTDINC_FLAGS)) \
                             -c -o $(IMAGE_VER) \
                             $(src)/utils/build_time/build_time.c

# Linker scripts preprocessor (.lds.S -> .lds)
# ---------------------------------------------------------------------------
quiet_cmd_cpp_lds_S = LDS     $@
      cmd_cpp_lds_S = $(CPP) $(call flags,KBUILD_CPPFLAGS) \
                             $(call flags,CPPFLAGS_$(@F)) \
                             -MD -MP -MF $(depfile) $(NOSTDINC_FLAGS) \
                             -P -C -o $@ $<

$(obj)/%.lds: $(src)/scripts/link/%.lds.S FORCE
...
ifeq ($(COMPILE),1)
	...
	+$(call if_changed2,image_ver,link-IMAGE_FILE)
endif

# The actual objects are generated when descending,
# make sure no implicit rule kicks in
$(sort $(filter-out $(LDS_FILE),$(IMAGE-deps))): $(IMAGE-builddirs) ;
