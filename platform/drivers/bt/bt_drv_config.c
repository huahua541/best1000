...
void btdrv_config_init(void)
{
    if(hal_get_chip_metal_id() >= HAL_CHIP_METAL_ID_2)
    {
        ...
        for(uint8_t i=0;i<sizeof(btdrv_cfg_tbl)/sizeof(btdrv_cfg_tbl[0]);i++)
        {
            ...
            {
                btdrv_cfg_tbl[i].is_act = BTDRV_CONFIG_ACTIVE;
            }
        }
    }
    else
...
{
	(BT_RFSPI_WR|BT_RFSPI_FRAME(1)|BT_RFSPI_BYTEPREFRAME(3)),
	0x0e,0x3a,0x04,
	//0x0e,0x3a,0x04,
	//0x1a,0x00,0x00,
};

uint8_t btdrv_bt_spi_neg_raw[]=
{
...
uint8_t btdrv_bt_spi_bitoffset_and_btclock_raw[]=
{
	(BT_RFSPI_WR|BT_RFSPI_FRAME(3)|BT_RFSPI_BYTEPREFRAME(3)),
...
uint8_t btdrv_ble_spi_pos_raw[16]=
{
	(BT_RFSPI_WR|BT_RFSPI_FRAME(3)|BT_RFSPI_BYTEPREFRAME(3)),
	0x18,0x00,0x7f,
	0x19,0x3C,0x40,
...
uint8_t btdrv_ble_spi_neg_raw[16]=
{
	(BT_RFSPI_WR|BT_RFSPI_FRAME(3)|BT_RFSPI_BYTEPREFRAME(3)),
	0x18,0x00,0x6c,
	0x19,0x24,0x40,
...
void btdrv_bt_spi_set_rawbuf(uint8_t wr_type, uint8_t *spirawbuf, uint8_t len)
{
	if (1 == wr_type)
		btdrv_write_memory(_8_Bit, EM_BT_SPIPOSBUF_ADRESS_BASE, spirawbuf, len);

	if (2 == wr_type)
		btdrv_write_memory(_8_Bit, EM_BT_SPINEGBUF_ADRESS_BASE, spirawbuf, len);

	if (3 == wr_type)
		btdrv_write_memory(_8_Bit, EM_BLE_SPIPOSBUF_ADRESS_BASE, spirawbuf, len);

	if (4 == wr_type)
		btdrv_write_memory(_8_Bit, EM_BLE_SPINEGBUF_ADRESS_BASE, spirawbuf, len);
}

void btdrv_bt_spi_rawbuf_init(void)
{
//	TRACE("%s\n", __func__);
//	btdrv_bt_spi_set_rawbuf(1, btdrv_bt_spi_pos_raw, sizeof(btdrv_bt_spi_pos_raw));
//	btdrv_bt_spi_set_rawbuf(2, btdrv_bt_spi_neg_raw, sizeof(btdrv_bt_spi_neg_raw));
//	btdrv_bt_spi_set_rawbuf(3, btdrv_ble_spi_pos_raw, sizeof(btdrv_ble_spi_pos_raw));
//	btdrv_bt_spi_set_rawbuf(4, btdrv_ble_spi_neg_raw, sizeof(btdrv_ble_spi_neg_raw));
