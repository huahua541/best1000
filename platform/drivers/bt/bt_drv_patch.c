...
BTDRV_PATCH_STRUCT data_patch1 =
{
    0
};


uint32_t data_patch_config[] =
{
    2,
    (uint32_t)&data_patch0,
    (uint32_t)&data_patch1,
...
void btdrv_data_patch_init(void)
{
    ...
    for(uint8_t i=0;i<data_patch_config[0];i++)
    {
        data_patch_p = (BTDRV_PATCH_STRUCT *)data_patch_config[i+1];
        if(data_patch_p->patch_state == BTDRV_PATCH_ACT)
            btdrv_data_patch_write((BTDRV_PATCH_STRUCT *)data_patch_config[i+1]);
    }
}
