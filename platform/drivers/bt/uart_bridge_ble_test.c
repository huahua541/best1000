...
static unsigned char uart_rx_data[1024];
static volatile unsigned int uart_rx_len = 0;

static unsigned char bt_rx_data[1024];
...
static unsigned int bt_rx(const unsigned char *data, unsigned int len)
{
    ...
    if (bt_rx_len + processed > sizeof(bt_rx_data)) {
        TRACE("bt_rx data too long: bt_rx_len=%d, len=%d, limit=%d", bt_rx_len, processed, sizeof(bt_rx_data));
        processed = sizeof(bt_rx_data) - bt_rx_len;
    }
    if (processed > 0) {
        memcpy(bt_rx_data + bt_rx_len, data, processed);
        bt_rx_len += processed;
        bt_rx_done = true;
    }
    return len;
...
void uart_bridge_ble_test(void)
{
    int ret;
    ...
    hal_uart_dma_recv(br_uart, uart_rx_data, sizeof(uart_rx_data), NULL, NULL);
    mask.reg = 0;
    mask.BE = 1;
    mask.FE = 1;
    mask.OE = 1;
    ...
    while (1) {
        if (uart_rx_done) {
            uart_rx_done = false;
            if (uart_rx_len > 0) {
                hal_intersys_send(br_intersys, HAL_INTERSYS_MSG_HCI, uart_rx_data, uart_rx_len);
            } else {
                bt_tx_done = true;
            }
        }
        ...
        if (bt_rx_done) {
            bt_rx_done = false;
            if (bt_rx_len > 0) {
                hal_uart_dma_send(br_uart, bt_rx_data, bt_rx_len, NULL, NULL);
            } else {
                uart_tx_done = true;
            }
        }
