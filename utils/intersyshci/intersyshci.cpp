...
#else
static unsigned short hci_h4_receive_msg( const uint8_t *buf, uint32_t size)
{
...
INTERSYSHCI_LOC static unsigned int bt_rx(const unsigned char *data, unsigned int len)
{
...
static uint32_t IAHCI_tx_flag = 0;
...
int IAHCI_SendBuffer(U8 packet_type, U8 *packet, int size)
{
    u8 buf[1024];
    buf[0] = packet_type;
